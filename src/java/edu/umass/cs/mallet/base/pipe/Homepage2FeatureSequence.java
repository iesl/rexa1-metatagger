...
public class Homepage2FeatureSequence extends Pipe
{

	private static final long serialVersionUID = 180922565564903719L;
	...
	public Homepage2FeatureSequence(String authorName, String authorEmail)
	{
	...
	{	
		...
		FeatureSequence result =
			new FeatureSequence(getDataAlphabet(), tmpStore.size());
		for(int i=0;i<tmpStore.size();i++)
			result.add(tmpStore.get(i));
	...
	{
		if(path.indexOf("REAL-HOMEPAGE-")!=-1)
			return true;
		return false;
	}
		...
		while(st.hasMoreTokens())
		{
			...
			if(targetToken.length()>4 && targetToken.substring(0,4).equals("font"))
			{
				...
				if(idx-5!=-1 && idx+2<=tmp.length())
				{
					...
					try
					{
						if(size.charAt(0)=='+' && (size.charAt(1)-'0') > 0)
							return true;
