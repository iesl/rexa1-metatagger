...
public class BibliographyLayoutSegmenter {

	private ArrayList reference_start_patterns;

	private static final int maxReferenceLength = 3200;
	private NewHtmlTokenization _tokenization;


	public BibliographyLayoutSegmenter(NewHtmlTokenization tokenization) {
	...
	private RegexReferenceHintList regexReferenceHintList(List lineList) {
		...
		if (lineIterator.hasNext()) {
			...
			Span firstLine = (Span)lineIterator.next();
			...
			while (lineIterator.hasNext()) {
				Span nextLine = (Span)lineIterator.next();
				lineIndex++;
				// If this line of text matches the reference pattern, insert a reference hint.
				Matcher currentMatcher = referencePatternUsed.matcher( nextLine.getText() );
				boolean isMatch = false;
				if (currentMatcher.find()) {
					// To guard against invalid matches, require sequentially increasing numbers for regex #6
					if (regexNum == 6) {
						int lineNum = Integer.parseInt( currentMatcher.group( 1 ) );
	...
	private double getSpanX(PropertyHolder span) {
		return span.getNumericProperty( "llx" );
	}
	...
	private double getSpanY(PropertyHolder span) {
		return span.getNumericProperty( "lly" );
	}

	/* Uses horizontal indentation to segment the references. This is still
	     in development. */

	/* check for negative dy use c/rattler.cameron.edu/http:\\rattler.cameron.edu\EMIS\journals\AG\1-4\4_323.pdf.xml as test*/
	...
	private HorizReferenceHintList horizReferenceHintList(List lineList) {
		...
		// Examine each line.
		while (lineIterator.hasNext()) {
			Span line = (Span)lineIterator.next();
			lineIndex++;
			if (isNewPage( (PropertyHolder)line )) {
				newcolumn = true;
			}

			double lineX = getSpanX( (PropertyHolder)line );
			double lineY = getSpanY( (PropertyHolder)line );

			prevLinePosition.x = curLinePosition.x;
			prevLinePosition.y = curLinePosition.y;
			curLinePosition.x = reverse ? -lineX : lineX;
			curLinePosition.y = lineY;
			...
			if (newcolumn) {
				...
				while (scanAheadIterator.hasNext() && scannedLines < 10) {
					Span scannedLine = (Span)scanAheadIterator.next();
					lineIndex++;

					if (isNewPage( (PropertyHolder)scannedLine )) {
						break;
					}

					double scannedLineX = getSpanX( (PropertyHolder)scannedLine );
					double scannedLineY = getSpanY( (PropertyHolder)scannedLine );
					...
					scannedLinePosition.x = reverse ? -scannedLineX : scannedLineX;
					scannedLinePosition.y = scannedLineY;
	...
	private VerticalReferenceHintList verticalReferenceHintList(List lineList) {
		...
		*/
		 ...
		 */
			...
			// todo else if (item instanceof TextPageBoundary) {
		...
		for (int j = 0; j < inLines.size(); j++) {
			...
			*/
			...
			for (int i = Math.max( 0, j - 20 ); i < j; i++) {
				Span obj2 = (Span)inLines.get( i );
				String str2 = obj2.getText();
	...
	private AlphabetReferenceHintList alphabetReferenceHintList(List lineList) {
		...
		if (lineIterator.hasNext()) {
			...
			while (lineI.hasNext()) {
				...
				////System.out.println( "alphabet hint: "+((StringSpan)lineList.get(lineIndex)).getText() );
				ret.add( new ReferenceHint( lineIndex ) );
			}
		}
		return ret;
	...
	private HintInfo addReferenceHints(List lineList) {
		...
		ret.hasRegexHints = (ret.regexHintList != null && ret.regexHintList.size() > 0);
		ret.hasHorizHints = (ret.horizHintList != null && ret.horizHintList.size() > 0);
		ret.hasVerticalHints = (ret.verticalHintList != null && ret.verticalHintList.size() > 0);
		ret.hasAlphabetHints = (ret.alphabetHintList != null && ret.alphabetHintList.size() > 0);
	...
	private ReferenceHintList combineReferenceHints(List lineList, HintInfo info) throws ReferenceParsingException {
		...
		* "out of order", StringSpan construction problems, etc) ocasionally the
