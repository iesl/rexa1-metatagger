...
import org.rexo.extraction.StringMultiSpan;
...
public class CitationContextExtractor {
	...
	CitationContextExtractor(List referenceList) {
	...
	// Find matches across region boundaries?
	private ArrayList findRegexMatchesInRegion(String regex, StringMultiSpan region) {
		ArrayList ret = new ArrayList();
		Pattern p = Pattern.compile( regex );

		for (int i = 0; i < region.size(); i++) {
			Span span = region.getComponent( i );
			String sourceText = span.getText();

			Matcher matcher = p.matcher( sourceText );
			//String refID = (String)((HSpans)referenceList.get(refNum)).getProperty("refID");
	...
	public void findRefData() {
		...
		for (int refNum = 0; refNum < referenceList.size(); refNum++) {
			...
			HSpans refSpans = (HSpans)referenceList.get( refNum );

			// Extract the reference marker if one is present
			String marker;
			HSpans markerSpans = refSpans.getChild( "ref-marker" );
			if (markerSpans != null) {
				String markerText = markerSpans.getText().trim();
				if (markerText.matches( validTagRegex )) {
					data.put( "marker", markerText );
					//log.debug("read ref marker: "+markerText);
				}
				else {
					// TODO: remove this log
					//log.info("Skipping invalid (extracted) ref marker: "+markerSpans.getText());
				}
			}
			else {
				//log.debug("no ref marker found: "+refSpans.getText());
			}

			// Extract the list of authors for this reference 
			List authorList = new LinkedList();

			HSpans authSpan = refSpans.getChild( "authors" );
			List lastNames = (authSpan == null) ? null : authSpan.getDescendants( "author-last" );
			//log.info("got "+(lastNames == null ? "0" : Integer.toString(lastNames.size()))+" elements.");
			if (lastNames != null && lastNames.size() > 0) {
				Iterator nameI = lastNames.iterator();
				while (nameI.hasNext()) {
					HSpans nameS = (HSpans)nameI.next();
					String lastName = nameS.getText()
							.replaceAll( "[.,:]", "" )
							.trim();
					if (lastName.matches( "[\\w\\-\\s']+" )) {
						//log.info("got last Name: "+lastName);
			...
			HSpans dateSpan = (HSpans)refSpans.getChild( "date" );
			if (dateSpan != null) {
				/* Get the date text */
	...
	public ArrayList findCitationsInContext(NewHtmlTokenization tokenization /*, StringMultiSpan sourceRegion */) throws CitationFinderException {
		...
		for (citationStyle = MAX_STYLE - 1; citationStyle > -1; citationStyle--) {
			...
			for (refNum = 0; refNum < referenceList.size(); refNum++) {
				...
				if (regex != null) {
					// todo : matches = findRegexMatchesInRegion( regex, sourceRegion );
				}
				else {
					matches = null;
				}
				...
				HSpans refSpans = (HSpans)referenceList.get( refNum );
				//log.info("found "+matches.size()+" matches using style "+citationStyle+", refNum="+refNum);
				//log.info("reference="+refSpans.getText());
				String refID = (String)refSpans.getProperty( "refID" );
				...
				for (int i = 0; i < matches.size(); i++) {
					...
					HSpans citation = new HSpans( "CitationContext", tokenization, contextWindow );
					citation.addChild( new HSpans( "PreMatch", tokenization, preMatchSpan ) );
					citation.addChild( new HSpans( "Match", tokenization, matchSpan ) );
					citation.addChild( new HSpans( "PostMatch", tokenization, postMatchSpan ) );
					//log.info("citation: "+citation.getText());
					//log.info("match: "+citation.getChild("CitationMatch").getText());
					citation.setProperty( "refersToRefID", refSpans.getProperty( "refID" ) );
					ret.add( citation );
				}
			}
			// If all references have been scanned without
			// encountering too many errors, return the result list
			if (refNum == referenceList.size())
				return ret;
		}
		throw new CitationFinderException( "Not enough citations in context found!" );
	}
}
