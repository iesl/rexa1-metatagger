...
public class BufferedPipedInputStream extends InputStream {
   ...
   * @since JDK1.1
   */
  protected static final int PIPE_SIZE = 1 * 1024 * 1024;
   ...
   */
  public BufferedPipedInputStream(BufferedPipedOutputStream src) throws IOException {
    connect( src );
  }
   ...
   * @throws IOException if an I/O error occurs.
   */
  public void connect(BufferedPipedOutputStream src) throws IOException {
    src.connect( this );
  }
  ...
  protected synchronized void receiveQuick(byte b[], int off, int len) throws IOException {
    if (!connected) {
      throw new IOException( "Pipe not connected" );
    }
    else if (closedByWriter || closedByReader) {
    ...
    System.arraycopy( b, in, buffer, out, len );
    // buffer[in++] = (byte)(b & 0xFF);
    if (in >= buffer.length) {
      in = 0;
    }
