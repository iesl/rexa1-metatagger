...

{
  ...
  if ( $options{setup} ) {
    mkdir $workroot unless -e $workroot;
    
  } else {

    -d 'dist-pstotext' or die "no pstotext distribution available";
    -d 'rexa-textmill' or die "no rexa-textmill distribution available";
    -d 'work' or die "work directory doesn't exist"; 

    my $pstotextbin = "./dist-pstotext/bin";
    my $taskid = 1;
    ...
    while ( 1 ) {
      ...
      runCmd( "./dist-cms/cms.pl --update-sources availability=pstotext.pending -Davailability=$nodeid --number $fetchCount");
	...
	my $file = "$fpath/$sha";
	my $rsyncURL = "rsync://adam@"."vinci5.cs.umass.edu/pspdf/$file*";
	...
	runCmd( "$rsyncCmd" );
      ...
      my $logfile = "$nodeWorkDir/totext.log.$nodeid";
      
      # count files
      runCmd( "(find $path/pdfs -type f -name '*.document' | wc --lines)" );
      # run totext
      runCmd( "find $path/pdfs -type f -name '*.document' -exec $pstotextbin/idftype --verbose --file {} ';'" );
      # determine file types/run pstotext
      runCmd( "find $path/pdfs -type f -exec $pstotextbin/shamunged-filename.pl --file {} --rename ';'" );
      runCmd( "find $path/pdfs -type f -name '*.ps' -exec $pstotextbin/totext --logprefix \"$machine:$taskid:\" --log $logfile --file {} ';'" );
      runCmd( "find $path/pdfs -type f -name '*.pdf' -exec  $pstotextbin/totext --logprefix \"$machine:$taskid:\" --log $logfile --file {} ';'" );

      ## remove all non-gzipped files (totext gzips files that are successfully extracted)
      runCmd( "find $path/pdfs -type f -not -name '*.gz' -exec rm -f {} ';' " );
      ## .. and then unzip the text files again
